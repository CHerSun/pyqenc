...
**Runner container-format fix (shared infrastructure).** The application pass DOES require one small, backward-compatible change to `ffmpeg_runner.py`. The runner writes every file output to a `.tmp` sibling first (atomicity); the `.tmp` extension strips the container hint ffmpeg would infer from the real extension, so the runner injects an explicit `-f <muxer>` for each `.tmp` output. It historically hardcoded `-f matroska`, which is correct for the video pipeline but wrong for audio (`.flac`, `.m4a`). The fix adds an optional `output_format: str | None = None` parameter to `run_ffmpeg` / `run_ffmpeg_async` (threaded through `_resolve_tmp_paths`): `None` preserves the `matroska` default so every existing video call site is unchanged, and the audio chain executor passes the correct muxer looked up from an extension→muxer map in `constants.py` (`OUTPUT_FORMAT_MUXERS`: `flac`→`flac`, `m4a`→`ipod`; an unmapped extension falls back to the Matroska default). Measurement passes still write no file (`output_file=None`) and are unaffected. This is the one shared-infrastructure change; it is explicit about the container per the project ffmpeg philosophy.
