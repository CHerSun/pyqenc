...
- `phase-object-model` established the `(config, phases)` constructor pattern. `app-metrics-report` extended it to `(config, phases, *, collector)`. `config-refactor` keeps that signature but changes `config` from `PipelineConfig` to `AppConfig`.
