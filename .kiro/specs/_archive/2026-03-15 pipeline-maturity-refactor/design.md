...
- The signal/atexit flush registered in this spec for `ProgressTracker` was superseded first by `phase-recovery-refactor` (state files) and later by `app-metrics-report` (SIGINT → `collector.flush(partial=True)`).
