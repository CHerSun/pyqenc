...
This refactor introduces a unified `QualitySearchProtocol` that encapsulates the full search state behind a single `record()` call. The existing algorithm is preserved as `QualitySearch`. A new `QualitySearchV2` implementing a 3-point sweet-spot search is added and becomes the default. `ChunkEncoder` is simplified to use the protocol interface exclusively.

Key design decisions:
- `QualitySearchProtocol` is a `typing.Protocol` -- structural subtyping, no inheritance required.
...
- `QualitySearchV2` is the default in `encode_chunk()`.
...
```
search = QualitySearchV2(quality_better, quality_worse, quality_targets, granularity, quality_max_step)
current_q = initial_quality
any_real_work = False

while True:
    existing = check_existing_encoding(current_q)
    if existing:
        ...
        any_real_work = True
        encode(current_q)                      # encode + measure
        metrics = evaluate()
...
finalize(search.best_quality, search.best_metrics, search.best_targets_met)
```
...
- `record()` updates internal state and returns the next quality value to try, or `None` when the search is exhausted or the current result is accepted.
- `quality_targets`, `granularity`, `quality_better`, `quality_worse`, and `quality_max_step` are supplied at construction time.
- Before any `record()` call: `best_quality` is `None`, `best_metrics` is `None`, `best_targets_met` is `False`.
...
Internal state: `_better_q`, `_worse_q`, `_better_metrics`, `_worse_metrics`, `_attempts`. Direction-agnostic: uses `quality_better`/`quality_worse` instead of assuming lower=better. The `record()` method runs the same proportional interpolation logic as `adjust_crf()`, using `_score_attempt` for the worst-target lookup.
...
- Search range: `[_pass_q(sentinel=quality_better) ... _best_q]`
- `_best_q` updated to each new better-scoring attempt; `_fail_q` lags one step behind as outer reserve
- Transition trigger: new attempt scores worse than `_best_q` -> `_pass_q = current` (3-point mode)

**All-passing case -- Phase 1** (while `_fail_metrics is None`):
- Search range: `[_best_q ... _fail_q(sentinel=quality_worse)]`
- `_best_q` updated to each new better-scoring attempt; `_pass_q` lags one step behind as outer reserve
- Transition trigger: new attempt scores worse than `_best_q` -> `_fail_q = current` (3-point mode)

**Phase 2 -- 3-point mode** (both `_pass_metrics` and `_fail_metrics` are not None):
- Two active ranges: Range A = `[_pass_q ... _best_q]`, Range B = `[_best_q ... _fail_q]`
- Next quality value: midpoint of the larger range (capped by `quality_max_step` if set)
...
Internal state:
- `_pass_q = quality_better` (sentinel)
- `_pass_metrics: dict[str, float] | None = None`
- `_best_q: Decimal = quality_worse` (sentinel -- updated to first real attempt)
- `_best_metrics: dict[str, float] | None = None`
- `_best_score: float = -inf`
- `_fail_q = quality_worse` (sentinel, lags behind `_best_q` in all-failing phase)
...
| >=1 call, none pass | quality value with smallest `abs(_score_attempt())` | metrics at that value | `False` |
| >=1 passing call | Best-efficiency passing quality value | metrics at that value | `True` |

`attempts` always equals the total number of `record()` calls.

### QualitySearchV2 State Transitions

On each `record(quality, quality_results)` call, compute `score = _score_attempt(quality_results, quality_targets)`.

If `score == 0.0` (early acceptance): update `_best_q`/`_best_metrics`/`_best_score`, set `_exhausted = True`, return `None`.
...
1. If `abs(score) < abs(_best_score)` (new best):
   - `_fail_q = _best_q`, `_fail_metrics = _best_metrics` (lag one step)
   - `_best_q = quality`, `_best_metrics = quality_results`, `_best_score = score`
   - Next: interpolate within `[_pass_q ... _best_q]`
2. If `abs(score) >= abs(_best_score)` (worse -- sweet spot passed):
   - **Transition**: `_pass_q = quality`, `_pass_metrics = quality_results`
   - Next: midpoint of larger of Range A and Range B

All-passing sub-case (`_fail_metrics is None`):
1. If `abs(score) < abs(_best_score)` (new best):
   - `_pass_q = _best_q`, `_pass_metrics = _best_metrics` (lag one step)
   - `_best_q = quality`, `_best_metrics = quality_results`, `_best_score = score`
   - Next: interpolate within `[_best_q ... _fail_q]`
2. If `abs(score) >= abs(_best_score)` (worse -- sweet spot passed):
   - **Transition**: `_fail_q = quality`, `_fail_metrics = quality_results`
   - Next: midpoint of larger of Range A and Range B

**Phase 2 -- 3-point mode** (both `_pass_metrics` and `_fail_metrics` are not None):

1. Determine range: Range A if `quality` is between `_pass_q` and `_best_q`, Range B if between `_best_q` and `_fail_q`.
2. If `abs(score) < abs(_best_score)` **and drawn from Range B**: promote -- `_pass_q = _best_q`, `_pass_metrics = _best_metrics`, `_best_q = quality`, `_best_metrics = quality_results`, `_best_score = score`, `_fail_q` unchanged.
3. If `abs(score) < abs(_best_score)` **and drawn from Range A**: demote outer -- `_fail_q = _best_q`, `_fail_metrics = _best_metrics`, `_best_q = quality`, `_best_metrics = quality_results`, `_best_score = score`, `_pass_q` unchanged.
4. If `abs(score) >= abs(_best_score)` **and drawn from Range B**: tighten -- `_fail_q = quality`, `_fail_metrics = quality_results`.
5. If `abs(score) >= abs(_best_score)` **and drawn from Range A**: tighten -- `_pass_q = quality`, `_pass_metrics = quality_results`.
6. Next quality value: midpoint of the larger of Range A and Range B (capped by `quality_max_step`).
...
*For any* valid `(quality_better, quality_worse, granularity)` where `quality_better != quality_worse` and `granularity > 0`, and for any sequence of quality results, `QualitySearch` must return `None` from `record()` in O(log N) attempts -- each attempt must halve the active search interval.

**Validates: Requirements 4.1**

### Property 2: QualitySearchV2 convergence

*For any* valid `(quality_better, quality_worse, granularity)` where `quality_better != quality_worse` and `granularity > 0`, and for any sequence of quality results, `QualitySearchV2` must return `None` from `record()` in O(log N) attempts -- each attempt must halve at least one of the two active ranges.

**Validates: Requirements 4.2**

### Property 3: Finality after exhaustion

*For any* `QualitySearchProtocol` implementation, once `record()` returns `None`, all subsequent `record()` calls must also return `None`.

**Validates: Requirements 4.3**

### Property 4: Protocol state invariants

*For any* `QualitySearchProtocol` implementation and any sequence of `record()` calls:
...
- When `best_targets_met` is `True`, `best_quality` is the best-efficiency passing quality value (smallest `abs(score)` among passing attempts).
- When `best_targets_met` is `False`, `best_quality` is the quality value of the attempt with the smallest `abs(_score_attempt())` value among all attempts.
...
|---|---|---|
| P1: QualitySearch convergence | `test_qualitysearch_convergence_bound` | `st.decimals` for range/granularity, `st.lists` of pass/fail booleans |
| P2: QualitySearchV2 convergence | `test_qualitysearchv2_convergence_bound` | same as P1 |
| P3: Finality after None | `test_search_finality_after_exhaustion` | parametrized over both implementations |
| P4: State invariants | `test_protocol_state_invariants` | parametrized over both implementations, `st.lists` of `(quality, quality_results)` pairs |
...
- `QualitySearch` state transitions: pass narrows better bound, fail narrows worse bound, exhaustion returns `None`.
- `QualitySearchV2` all state-transition cases: one test per case with a concrete setup.
- `QualitySearchV2` initial sentinel state.
...
- `encode_chunk` uses `QualitySearchV2` by default -- verified via mock.
- `QualitySearch` usable as drop-in alternative.
- Fully-recovered chunk (all cache hits) advances progress bar as SKIPPED.

### Integration Tests

`tests/integration/test_encoding_quality.py` (extended): verify that `encode_chunk` with a mocked quality evaluator converges correctly using `QualitySearchV2`, including artifact-check-per-step recovery and progress bar SKIPPED behavior.
