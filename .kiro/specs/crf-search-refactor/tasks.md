...
- [x] 1. Add `MetricInfo.acceptance_delta` and remove `CRF_METRIC_POSITIVE_DELTA`
...
- [x] 2. Add `CodecConfig.quality_log_padding` and remove `PADDING_QUALITY_NUMBER`
  ...
  - [x] 2.1 Write unit tests for `CodecConfig.quality_log_padding`
    - Test CRF range `[0, 51]` gran `0.5` → `4`
    - Test VBR range `[0, 100]` gran `0.1` → `5`
    - Test QP range `[0, 63]` gran `1` → `2`
    - _Requirements: 10.7_

- [x] 3. Update attempt filename constants in `pyqenc/constants.py`
...
- [x] 4. Update `ChunkEncoder` filename construction and regex callers in `pyqenc/phases/encoding.py`
...
- [x] 5. Implement `_score_attempt()` in `pyqenc/quality.py`
  ...
  - [x] 5.1 Write unit tests for `_score_attempt` in `tests/unit/test_quality.py`
  ...
  - [x] 5.2 Write property test P5: `_score_attempt` sign contract in `tests/test_crf_search_properties.py`
...
- [x] 6. Implement `QualitySearchProtocol` in `pyqenc/quality.py`
...
- [x] 7. Implement `QualitySearch` class in `pyqenc/quality.py`
  ...
  - [x] 7.1 Write unit tests for `QualitySearch` in `tests/unit/test_quality.py`
  ...
  - [x] 7.2 Write property test P1: `QualitySearch` convergence in `tests/test_crf_search_properties.py`
...
- [x] 8. Implement `QualitySearchV2` class in `pyqenc/quality.py`
  ...
  - [x] 8.1 Write unit tests for `QualitySearchV2` state transitions in `tests/unit/test_quality.py`
  ...
  - [x] 8.2 Write property test P2: `QualitySearchV2` convergence in `tests/test_crf_search_properties.py`
  ...
  - [x] 8.3 Write property test P3: finality after exhaustion in `tests/test_crf_search_properties.py`
  ...
  - [x] 8.4 Write property test P4: protocol state invariants in `tests/test_crf_search_properties.py`
...
- [x] 10. Refactor `ChunkEncoder.encode_chunk()` in `pyqenc/phases/encoding.py`
  ...
  - [x] 10.1 Write unit tests for `encode_chunk` integration in `tests/unit/test_quality.py`
  ...
  - [x] 10.2 Write integration tests in `tests/integration/test_encoding_quality.py`
