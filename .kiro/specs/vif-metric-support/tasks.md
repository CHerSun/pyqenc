...
- [~] 1. Update `MetricInfo` docstring and `QualityArtifacts` dataclass
  - [~] 1.1 Update `MetricInfo` docstring in `pyqenc/quality.py` to replace `scale_factor` → `_scale_factor`, `clip_lower` → `_clip_lower`, `clip_upper` → `_clip_upper`, and document `_offset`
    - No code changes — fields and `normalize()` are already correct
    - _Requirements: 1.8_
  - [~] 1.2 Update `QualityArtifacts` in `pyqenc/quality.py`: add `vif_log: Path | None = None` field; remove `stats_files: list[Path]` field
    - _Requirements: 6.4, 12.6_

- [~] 2. Add VIF branch to `run_metric` and update `QualityTarget.parse`
  - [~] 2.1 Add `elif metric == MetricType.VIF:` branch in `run_metric` (`pyqenc/quality.py`) before the `assert_never(metric)` line
  ...
  - [ ] 2.2 Replace hardcoded `valid_metrics = {"vmaf", "ssim", "psnr"}` in `QualityTarget.parse` (`pyqenc/models.py`) with `{m.value for m in MetricType}`
    - _Requirements: 7.1, 7.2, 7.3_
  - [ ] 2.3 Write property test for `QualityTarget.parse` VIF acceptance
    - **Property 12: `QualityTarget.parse` accepts VIF**
    - **Validates: Requirements 7.1, 7.3**
    - Place in `tests/test_vif_properties.py`

- [ ] 3. Implement VIF log parser in `pyqenc/utils/visualization.py`
  - [ ] 3.1 Add `_parse_vif_line(line: str) -> dict[str, float] | None` helper
  ...
  - [ ] 3.2 Add `parse_vif_file(file_path: Path, factor: int = 1) -> pd.DataFrame`
  ...
  - [ ] 3.3 Write property test: `parse_vif_file` DataFrame structure
    - **Property 6: `parse_vif_file` DataFrame structure**
    - **Validates: Requirements 4.1, 4.2**
  - [ ] 3.4 Write property test: `parse_vif_file` zero-based frame indexing
    - **Property 7: `parse_vif_file` zero-based frame indexing**
    - **Validates: Requirements 4.3**
  - [ ] 3.5 Write property test: `parse_vif_file` finite values
    - **Property 8: `parse_vif_file` finite values**
    - **Validates: Requirements 4.6**
  - [ ] 3.6 Write property test: `parse_vif_file` skips malformed lines
...
- [ ] 5. Integrate VIF into `analyze_chunk_quality`
  - [ ] 5.1 Add `vif_log: Path | None = None` and `delete_after_parse: bool = True` parameters to `analyze_chunk_quality` in `pyqenc/utils/visualization.py`
  ...
  - [ ] 5.2 Write property test: `analyze_chunk_quality` VIF normalization
    - **Property 10: `analyze_chunk_quality` VIF integration and normalization**
    - **Validates: Requirements 5.2, 5.3**
  - [ ] 5.3 Write property test: `analyze_chunk_quality` backward compatibility
    - **Property 11: `analyze_chunk_quality` backward compatibility**
    - **Validates: Requirements 5.5**

- [ ] 6. Add VIF to `DEFAULT_METRIC_STYLES` and update `create_unified_plot`
  - [ ] 6.1 Add `_VIF_COLOR: str = "#7B2D8B"` and `_VIF_FILL_COLOR: str = "#C084D4"` constants in the constants section of `pyqenc/utils/visualization.py`
  ...
  - [ ] 6.2 Update `create_unified_plot` in `pyqenc/utils/visualization.py`
...
- [ ] 7. Update `QualityEvaluator` — `_generate_metrics`, `_finish_evaluation`, `evaluate_chunk_async` / `evaluate_chunk`
  - [ ] 7.1 Update `QualityEvaluator._generate_metrics` in `pyqenc/utils/visualization.py`
  ...
  - [ ] 7.2 Update `QualityEvaluator._finish_evaluation` in `pyqenc/utils/visualization.py`
  ...
  - [ ] 7.3 Update `QualityEvaluator.evaluate_chunk_async` and `evaluate_chunk` in `pyqenc/utils/visualization.py`
  ...
  - [ ] 7.4 Write property test: `_generate_metrics` returns `.tmp` paths
    - **Property 15: `_generate_metrics` returns `.tmp` paths directly**
    - **Validates: Requirements 6.2, 12.1, 12.2**

- [ ] 8. Update `run_measure` in `pyqenc/phases/measure.py`
  - [ ] 8.1 Remove `metrics_dirs` dict and all references; remove `METRICS_SUBDIR_SUFFIX` import from inside `run_measure`
...
- [ ] 9. Update `pyqenc/cli.py` — `_QUALITY_TARGET_HELP` constant
  - [ ] 9.1 Define `_QUALITY_TARGET_HELP: str` constant before `_add_quality_arguments`
...
- [ ] 11. Write remaining property-based tests in `tests/test_vif_properties.py`
  - [ ] 11.1 Write property test: normalization formula correctness
  ...
  - [ ] 11.5 Write property test: `passes()` direction correctness
  ...
  - [ ] 11.6 Write property test: target evaluation direction-awareness
  ...
  - [ ] 11.7 Write property test: sidecar YAML VIF key generation
